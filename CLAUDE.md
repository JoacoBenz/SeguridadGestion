...
- Sentry / error tracking (not wired — add `@sentry/nextjs` when ready). `error.tsx` / `global-error.tsx` already log the `digest`, which is the only handle that ties what the user saw to the server log line.
