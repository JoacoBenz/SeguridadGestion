...
- **Device-PIN flow is implemented** (see "Device-PIN" under Operational pieces): a long-lived signed cookie per conserjería device, unlocked by a per-building PIN. `getSession()` falls back to it as a `guard` session. Guards can still sign in by email.
...
- **The bucket is public-read** because Meta fetches the image when building the WhatsApp message. The UUID makes URLs unguessable, but there is no access control and no tenant isolation at the storage layer — anyone with the link gets the photo, and a package label usually shows the resident's name and address. Don't treat `photoUrl` as private.
- **Per-tenant photo policy** lives in `Tenant.settings` and is edited from `/[tenant]/admin`: `photoMode` (`required` | `optional` | `disabled`, default `required`), `photoRetentionDays` (default 60, `0` = keep forever) and `conserjeriaPhone` (optional — sends a `paquete_foto_v1` copy to the front-desk phone so the guard keeps the photo in their own WhatsApp, which outlives the retention window; empty = no copy). Pure rules in `src/lib/photo-policy.ts` (unit-tested) — `shouldShowPhotoField()` / `isPhotoRequired()` also fold in whether storage is configured at all, so callers ask those two rather than combining flags themselves.
- `PhotoCapture` uploads on file pick and lets the guard retake ("Sacar de nuevo") or drop ("Quitar") the shot — the movida-photo case is the common one at the desk. Replacing or removing calls `DELETE /api/upload?slug=&url=`, so the discarded object goes immediately instead of waiting for the orphan sweep. That route re-checks the role, that the key sits under `packages/{tenantId}/` (otherwise a guard could delete another building's photos) and that no `Package` references it (409 — deleting a registered package's photo is not a discard). A sequence ref in the component keeps an older in-flight upload from overwriting a newer one.
- **Photos are ephemeral by default** (`Tenant.settings.photoEphemeral`, default `true`): `registerPackage` deletes the object right after the WhatsApp sends and nulls `photoUrl`, so the permanent copy is the resident's (and optionally the front desk's) chat, not our public bucket. It only deletes if at least one photo message went out — a total send failure keeps the file so it can be diagnosed.
...
### Device-PIN (shared conserjería device)
