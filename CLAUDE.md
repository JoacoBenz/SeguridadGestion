...
- **Photo policy is system behaviour, not a per-building setting.** `src/lib/photo-policy.ts` (unit-tested) fixes it: the photo is asked for whenever storage is configured (`isPhotoRequired`), it is kept so the admin can see it in `/admin/paquetes`, and it is deleted `PHOTO_RETENTION_DAYS` (30) after the package closes. The only per-tenant value is `Tenant.settings.conserjeriaPhone` — the front-desk number that gets a `paquete_foto_v1` copy, which only the admin knows. Don't reintroduce required/optional/disabled toggles.
