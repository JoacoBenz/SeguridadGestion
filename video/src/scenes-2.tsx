...
export const WhatsAppScene: React.FC<SceneProps> = ({ vertical }) => {
  ...
  const phone = (
    <div
      style={{
        flex: vertical ? undefined : "0 0 560px",
        opacity: phoneIn,
  ...
  return (
    <Bg>
      <AbsoluteFill
        style={
          vertical
            ? { flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 35, gap: 56 }
            : { flexDirection: "row", alignItems: "center", padding: 90, gap: 80 }
        }
      >
        {narrative}
        {phone}
...
export const Retiro: React.FC<SceneProps> = ({ vertical }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  const mockup = (
    <div
      style={{
        flex: vertical ? undefined : "0 0 980px",
        width: vertical ? 980 : undefined,
        position: "relative",
        opacity: mockIn,
        transform: `translateY(${(1 - mockIn) * 60}px)`,
        marginBottom: vertical ? 50 : 0,
      }}
    >
...
const Kpi: React.FC<{
  ...
  index: number;
  half?: boolean;
}> = ({ label, target, suffix = "", color = C.text, index, half }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
export const Dashboard: React.FC<SceneProps> = ({ vertical }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <Bg>
      <AbsoluteFill
        style={{
          justifyContent: "center",
          padding: vertical ? "0 56px" : "0 130px",
          gap: 44,
        }}
      >
        ...
        <div style={{ display: "flex", gap: 26, flexWrap: vertical ? "wrap" : "nowrap" }}>
          <Kpi label="Pendientes" target={12} color={C.accent} index={0} half={vertical} />
          <Kpi label=">3 días" target={2} color={C.warn} index={1} half={vertical} />
          <Kpi label="Recibidos / mes" target={248} index={2} half={vertical} />
          <Kpi label="Retirados / mes" target={236} color={C.positive} index={3} half={vertical} />
        </div>
...
export const Cierre: React.FC<SceneProps> = ({ vertical }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <Bg>
      ...
      >
        ...
        >
          <LogoIcon size={vertical ? 100 : 120} />
          <Wordmark size={vertical ? 92 : 110} okProgress={logoIn} />
        </div>
        <div
          style={{
            opacity: ctaIn,
