  ...
  return (
    <Bg>
      ...
      >
        ...
        >
          <LogoIcon size={150} />
          <Wordmark size={140} okProgress={okIn} />
        </div>
        <div
          style={{
            fontFamily: FONT,
...
const ProblemCard: React.FC<{ text: string; appearFrame: number }> = ({
  text,
  appearFrame,
}) => {
  const frame = useCurrentFrame();
  ...
  return (
    ...
    >
      <Card style={{ padding: "34px 60px", backgroundColor: C.cardAlt }}>
        <span style={{ fontFamily: FONT, fontSize: 46, fontWeight: 600, color: C.textSoft }}>
          {text}
        </span>
      </Card>
      <div
  ...
  return (
    <Bg>
      <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", gap: 42 }}>
        ...
        <ProblemCard text="¿Paquetes anotados en un cuaderno?" appearFrame={14} />
        <ProblemCard text="¿Residentes que nunca se enteran?" appearFrame={26} />
        <ProblemCard text="¿Retiros sin ningún registro?" appearFrame={38} />
      </AbsoluteFill>
    </Bg>
  );
};
